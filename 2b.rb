...
def checksum(row)
  selection = row.select{ |x| x.even? }
  row.each do |number|
    selection = row.dup
    selection.delete(number)

    if other_number = selection.find{ |n| number % n == 0 }
      return number / other_number
    end
  end
end

if checksum(numbers[0]) != 4
  raise "incorrect code"
end

result = numbers.map{ |n| checksum(n)}.sum
...
result = input.map{|n| checksum(n)}.sum
puts result
# 261
